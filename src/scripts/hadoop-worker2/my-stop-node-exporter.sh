...
echo "stop node-exporter on hadoop-worker2 container"
netstat -nltp | grep ':9100' | awk '{print $7}' | cut -d'/' -f1 | xargs -r kill -15

while true; do
  if ! nc -z localhost $PORT; then
      echo "hadoop-worker2 container: port 9100 is already down"
      echo "hadoop-worker2 container: successfully stop node-exporter on hadoop-worker2 container"
      break
    fi
    sleep 1
    COUNT=$((COUNT+1))
    if [ $COUNT -ge $MAX_WAIT ]; then
      echo "hadoop-worker2 container: Timeout"
      echo "hadoop-worker2 container: FAILED to stop node-exporter on hadoop-worker2 container"
      exit 1
    fi
done