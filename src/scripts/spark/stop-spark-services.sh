...
echo "Spark container: stop spark services: ThriftServer"
...
while true; do
  if ! nc -z localhost $PORT; then
    echo "spark container: port 10000 is already down"
    echo "spark container: finished stop spark services"
    break
  fi
  sleep 1
  COUNT=$((COUNT+1))
  if [ $COUNT -ge $MAX_WAIT ]; then
    echo "spark container: Timeout: port $PORT is still open"
    echo "spark container: cannot stop the spark ThriftServer"
    exit 1
  fi
done
